...
fc <- read.table(
  "/Users/qianzhao/Desktop/academics/thesis/data/aim 3/gene_counts.nameSorted.txt",
  header = TRUE,
  sep = "\t",
  comment.char = "#",
...
#clean sample names
colnames(countData) <- sub("\\.nameSorted\\.bam$", "", colnames(countData))
...
df <- df %>%
  mutate(
    ...
    status = case_when(
        ...
        log2FoldChange >= 1  ~ "Upregulated in KPC",
        #downregulated in KPC = significant, log2FC <= -1, mean expression >= 1e2
        baseMean_plot >= mean_cutoff &
        !is.na(padj) & padj < 0.05 &
        log2FoldChange <= -1 ~ "Downregulated in KPC",
      TRUE ~ "Filtered Out"
    )
  )
